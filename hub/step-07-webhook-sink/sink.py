...
def claim_slot(name):
    ...
    with LEDGER_LOCK:
        ...
        if key in led["claims"]:
            ...
            idx = led["next"]
            if idx >= len(relax):
                return None, ("every credential has been claimed — "
                              "tell the instructor, there are spares")
    ...
    return {
        ...
        "slot": i + 1,
        "code": slot["code"],
        # A single key in the pool means everyone shares it; that is a
        # deliberate fallback, not a bug.
        "relax_key": relax[i] if i < len(relax) else relax[-1],
        "mcp_token": tokens[i] if i < len(tokens) else (tokens[-1] if tokens else ""),
    }, None
